...
> **已确认（2026-06-30）**：`scorer.py` 原 lines 98–127 产品碳足迹计算块为错误引入的 bug，已完整删除。
...
| 行业样本量为 0 或 1 | 所有线性指标得满分（percentile=1），同时在 `flags` 中追加警告："行业样本数量仅为 N，线性指标得满分，评级结果参考价值有限" |
...
> **已确认（2026-06-30）**：`execute_finalize_footprint()` 在调用 `summarize_footprint` 前自动构建 `assumptions` 列表。自动写入条件：①原材料有未知因子（跳过的材料名）；②包装材料有未知因子；③运输距离>0 且运输方式不在已知列表（自动使用公路默认因子）；④电网大区无法匹配（使用全国平均因子）。

#### 2d. 结果展示与下载

- 计算完成后，聊天界面追加一个**永久**的摘要气泡（含总排放、类比行驶里程、最大热点来源及占比）和下载按钮。气泡不会被后续计算替换或移除。
- 每次计算生成唯一 `calc_id`（格式：`{session_id}__c{n}`）。下载链接指向 `/footprint-report/{calc_id}`，永久可访问。
- 后端同时维护 `all_calc_results[calc_id]`（永不覆盖）和 `calc_results[session_id]`（最新结果，供 `/compliance-check` 端点使用）。

> **已确认（2026-06-30）**：每次计算各自可访问，下载 UI 气泡永久保留在对话框中。
